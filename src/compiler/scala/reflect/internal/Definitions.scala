...
trait Definitions extends reflect.api.StandardDefinitions {
  ...
  object definitions extends AbsDefinitions with ValueClassDefinitions {
    ...
    lazy val CodeModule            = getModule(sn.Code)
         ...
         def Code_ulift            = getMember(CodeModule, nme.ulift_)
         def Code_splice           = getMember(CodeModule, nme.splice_)
         def Code_usplice          = getMember(CodeModule, nme.usplice_)
         def Code_typed            = getMember(CodeModule, nme.typed_)
    lazy val UcodeClass            = getClass(sn.Ucode)
    lazy val UcodeModule           = getModule(sn.Ucode)
         def Ucode_lift            = getMember(UcodeModule, nme.lift_)
         def Ucode_splice          = getMember(UcodeModule, nme.splice_)
