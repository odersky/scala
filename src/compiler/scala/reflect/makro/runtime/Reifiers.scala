...
trait Reifiers {
  ...
  def reifyTopLevel(any: Any, forImplicit: Boolean = false, mustBeGround: Boolean = false) = {
    class Reifier {
       ...
       *      ...
       *      $localN = new TermSymbol(ownerN, NoPosition, nameN)
       *      $local1.setInfo(tpe1)
       *      ...
       *      $localN.setInfo(tpeN)
       ...
       *   - `rtree` is code that generates `data` at runtime.
       *   - `data` is a tree or a type.
      ...
      private def reifyArray(xs: Array[_]): Tree =
        // doesn't work for Array(LiteralAnnotArg(...))
            ...
            if (sym == NoSymbol)
              ...
              if (sym.isTerm) {
                ...
                def markIfCaptured(arg: Ident): Tree =
                  if (sym.isCapturedVariable) referenceCapturedVariable(arg.symbol) else arg
      ...
      private def trimTree(tree: Tree): Tree = {
        ...
        def trimSyntheticCaseClassCompanions(stats: List[Tree]) =
          stats diff (stats collect { case moddef: ModuleDef => moddef } filter (moddef => {
            ...
            // we also have to do this ugly hack for the very same reason described above
      ...
      private def reifyTypeTree(tt: TypeTree): Tree = {
        if (definedInLiftedCode(tt.tpe)) {
          ...
          if (tt.original != null) {
            ...
            class AnnotationFixup extends Transformer {
          ...
          // temporarily disabling reification of originals
      ...
      private def reifyAnnotation(ann: AnnotationInfo): Tree =
        // if you reify originals, you get SO when trying to reify AnnotatedTypes, so screw it - after all, it's not that important
