...
trait MethodSynthesis {
  ...
  trait MethodSynth {
    ...
    def addDerivedTrees(typer: Typer, stat: Tree): List[Tree] = stat match {
      case vd @ ValDef(mods, _, _, _) if !noFinishGetterSetter(vd) =>
        ...
        // Martin -> Paul: Why not add if !vd.symbol.isLazy to the guard of this
        // case instead? It looks strange that we build annotations and trees
 ...
 *       case cd @ ClassDef(mods, _, _, _) if mods.isImplicit =>
        ...
        List(cd, wrapper.derivedTree setSymbol wrapper.derivedSym)
 */
      case _ =>
        List(stat)
    }
    ...
    protected def enterBeans(tree: ValDef) {
      ...
      if (beans.nonEmpty) {
        ...
        // Create and enter the symbols here, add the trees later in addDerivedTrees.
